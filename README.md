...
## Prerequisites 
Prerequisites, in the default configuration, you will need docker installed on the machine and a GPU with at least 20GB of VRAM (e.g. RTX 3090 or 4090).  Technically you can fit Qwen/Qwen3-14B-FP8 on a 16GB GPU, but will have little to no space left over for context.

You will also need NVIDIA's container toolkit installed
...
## Startup
to start, from the root of the repo run
```
docker compose up -d
```
...
On first start, a directory named `model` will be created, this is made available to the vllm container via a bind mount, it is used to store the model when downloaded.  A docker volume is used for the web-ui config.  There is no persistent storage configured for prom or grafana.
...
To tear down and remove volumes too (will will not delete the model directory)
```
